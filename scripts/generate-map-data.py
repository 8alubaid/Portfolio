#!/usr/bin/env python3
"""
Regenerates js/world-data.js and js/highlight-data.js from a public-domain
world country boundaries dataset. Run this only if you want to change the
simplification tolerance, add/remove highlighted countries, or refresh the
source data — the generated files are already committed, so this script
isn't part of the site's runtime or build.

Usage:
    pip install requests   # only dependency beyond the standard library
    python scripts/generate-map-data.py

Source data: johan/world.geo.json (MIT), fetched fresh each run from GitHub.
"""
import json
import math
import os
...
# Countries to break out into js/highlight-data.js, with a short key used by
